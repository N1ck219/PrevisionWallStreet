...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - LIVE V7.3 - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler(os.path.join(BASE_DIR, "reports", "live_v7_3_log.txt"), encoding='utf-8'),
...
from strategies.v7.strategy_v7_3 import StrategyV73

def main():
    logging.info("=== AVVIO BOT V7.3 IN LIVE LOOP ===")
    ...
    while True:
        try:
            ...
            if 570 <= total_minutes <= 960:
                logging.info(f"⏳ Avvio esecuzione V7.3...")
                
                # Esegue la strategia aggiornando DB e log
                bot.run()
                
                logging.info(f"✅ Ciclo V7.3 completato.")
