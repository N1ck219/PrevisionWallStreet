"""
strategy_v7_2.py — Strategia Intraday Split Brain V7.2.

Strategia di trading intraday in reale, derivata dalla V7.0, con:
...
class StrategyV72(BaseStrategy):
    bot_name = "V7.2 INTRADAY PRO (Split Brain 5min + Dynamic Exit)"
    ...
    def setup_trades_db(self):
        ...
        self.conn_trades.execute('''CREATE TABLE IF NOT EXISTS state_v72 (
        ...
        self.conn_trades.execute('''CREATE TABLE IF NOT EXISTS trades_history_v72 (
        ...
        for t in TARGET_TICKERS_V70:
            if not self.conn_trades.execute(
                "SELECT count(*) FROM state_v72 WHERE Ticker=?", (t,)
            ).fetchone()[0]:
                self.conn_trades.execute(
                    "INSERT INTO state_v72 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def _load_state(self, ticker):
        row = self.conn_trades.execute(
            "SELECT entry, direction, qty, sl, tp, highest, lowest, status, stage, entry_time, invested FROM state_v72 WHERE Ticker=?",
            (ticker,)
        ).fetchone()
        return {
    ...
    def _save_state(self, ticker, s):
        self.conn_trades.execute(
            "UPDATE state_v72 SET entry=?, direction=?, qty=?, sl=?, tp=?, highest=?, lowest=?, status=?, stage=?, entry_time=?, invested=? WHERE Ticker=?",
    ...
    def _record_trade(self, ticker, direction, entry_price, exit_price, qty, pnl, entry_time, exit_time, reason):
        ...
        self.conn_trades.execute(
            "INSERT INTO trades_history_v72 (ticker, direction, entry_price, exit_price, qty, pnl, entry_time, exit_time, exit_reason) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def _log_predictions(self, results):
        ...
        csv_path = os.path.join(os.path.dirname(self.db_market_path), "..", "reports", "v7_2_predictions_log.csv")
    ...
    def execute(self):
        ...
        for ticker in tqdm(TARGET_TICKERS_V70, desc="🎯 Analisi Intraday V7.2"):
        ...
        win_rate_str = ""
        try:
            total = self.conn_trades.execute("SELECT count(*) FROM trades_history_v72").fetchone()[0]
            wins = self.conn_trades.execute("SELECT count(*) FROM trades_history_v72 WHERE pnl > 0").fetchone()[0]
            if total > 0:
        ...
        mode = 'SOLO CHIUSURE' if close_only else ('SEGNALI 5min' if is_5min else 'MONITORAGGIO SL/TP (V7.2)')
    ...
    def send_report(self):
        ...
        # Invia solo alle 16:00 ET (fine giornata EOD)
        if total_min == 960:
            ...
            try:
                ...
                cursor = conn_tmp.execute(
                    "SELECT ticker, direction, pnl, exit_reason FROM trades_history_v72 WHERE exit_time LIKE ?",
                    (f"{today_str}%",)
                )
