...
def setup_db(conn):
    conn.execute('''CREATE TABLE IF NOT EXISTS state_v64 (Ticker TEXT PRIMARY KEY, entry REAL, atr_entry REAL, highest REAL, lowest REAL, invested REAL, pos INTEGER, half_sold INTEGER, qty INTEGER)''')
    for t in TARGET_TICKERS_AZIONARIO:
        if not conn.execute("SELECT count(*) FROM state_v64 WHERE Ticker=?", (t,)).fetchone()[0]:
            conn.execute("INSERT INTO state_v64 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (t, 0.0, 0.0, 0.0, 999999.0, 0.0, 0, 0, 0))
    conn.commit()

def load_portfolio_state(conn, ticker):
    row = conn.execute("SELECT entry, atr_entry, highest, lowest, invested, pos, half_sold, qty FROM state_v64 WHERE Ticker=?", (ticker,)).fetchone()
...
def save_portfolio_state(conn, ticker, s):
    conn.execute("UPDATE state_v64 SET entry=?, atr_entry=?, highest=?, lowest=?, invested=?, pos=?, half_sold=?, qty=? WHERE Ticker=?", (s['entry'], s['atr_entry'], s['highest'], s['lowest'], s['invested'], s['pos'], int(s['half_sold']), s['qty'], ticker))
    conn.commit()

def run():
    print(f"🚀 AVVIO LIVE BOT V6.4 APEX FUND (Macro Oracle) - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}")
    import sqlite3
    ...
    model = get_model("6.4", MODEL_NAME, shape_t=(LOOKBACK_DAYS, t_feat.shape[1]), shape_m=(LOOKBACK_DAYS, m_feat.shape[1]))
    ...
    app_state = {t: load_portfolio_state(conn, t) for t in TARGET_TICKERS_AZIONARIO}
    opportunities = []
