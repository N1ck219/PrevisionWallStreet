...
REPORT_DIR = os.path.join(BASE_DIR, 'reports', 'v7_1')
...
def run_backtest(test_days=TEST_DAYS):
    """Esegue il backtest intraday V7.1 su barre a 5 minuti."""
    print(f"\n🚀 AVVIO BACKTEST V7.1 (SMART EXIT 5min) — {test_days} Giorni")
    print("=" * 60)
    ...
    for data_odierna in tqdm(test_dates, desc="Simulazione Giorni"):
        ...
        for h, m in time_slots:
            ...
            for ticker, pos in posizioni_attive.items():
                ...
                if is_eod:
                    ...
                    pnl = (prezzo - pos['entry']) * pos['qty'] * pos['direction']
                    cash += pos['qty'] * pos['entry'] + pnl # Ritorno capitale investito + PnL
                    tutti_trade.append({
                        'ticker': ticker, 'date': str(data_odierna),
                        'direction': pos['direction'], 'entry': pos['entry'], 'exit': prezzo,
                        'pnl': pnl, 'reason': 'EOD', 'hour': h
                    })
                ...
                # Take Profit
                if pos['status'] != 'trailing' and rend >= pos['tp']:
                    pnl = (prezzo - pos['entry']) * pos['qty'] * pos['direction']
                    cash += pos['qty'] * pos['entry'] + pnl
                    tutti_trade.append({
                        'ticker': ticker, 'date': str(data_odierna),
                        'direction': pos['direction'], 'entry': pos['entry'], 'exit': prezzo,
                        'pnl': pnl, 'reason': 'TP', 'hour': h
                    })
                    if h in trade_per_ora: trade_per_ora[h] += 1
                    tickers_to_close.append(ticker)
                    continue
            ...
            if h < 15 and not is_eod:
                ...
                for s in segnali:
                    ...
                    qty = int(budget // s['prezzo'])
                    
                    if qty > 0 and budget > 100:
                        direction = 1 if s['delta'] > 0 else -1
                        baseline_sl = (2.0 * s['atr'] / s['prezzo']) if s['atr'] > 0 else 0.02
        ...
        for ticker, pos in list(posizioni_attive.items()):
            pnl = (pos['last_price'] - pos['entry']) * pos['qty'] * pos['direction']
            cash += pos['qty'] * pos['entry'] + pnl
            tutti_trade.append({
                'ticker': ticker, 'date': str(data_odierna),
                'direction': pos['direction'], 'entry': pos['entry'], 'exit': pos['last_price'],
...
def generate_report(storia_equity, tutti_trade, trade_per_ora, test_dates):
    ...
    print("\n" + "=" * 60)
    print("📊 REPORT BACKTEST V7.1 (SMART EXIT)")
    print("=" * 60)
    ...
    fig.suptitle('Backtest V7.1 — Smart Exit (5-min Bars)', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    report_path = os.path.join(REPORT_DIR, "backtest_v7_1_report.png")
    ...
    if n_trade > 0:
        csv_path = os.path.join(REPORT_DIR, "trades_v7_1.csv")
